...
class WeiboJsonStoreImplement(AbstractStore):  
    ...
    def __init__(self):  
        super().__init__()  
        # 不再需要run_time_tag，我们将使用关键词和时间范围来组织文件  
  
    def make_save_file_name(self, store_type: str, keyword: str = None, time_range: str = None) -> tuple[str, str]:  
        """  
        ...
        """  
        # 如果没有提供关键词，则使用当前的crawler_type  
        if not keyword:  
            keyword = crawler_type_var.get()  
          
        # 如果没有提供时间范围，则使用当前日期  
        if not time_range:  
            time_range = utils.get_current_date()  
              
        # 创建基于关键词的子目录  
        keyword_dir = os.path.join(self.json_store_path, keyword.replace(" ", "_"))  
        keyword_words_dir = os.path.join(self.words_store_path, keyword.replace(" ", "_"))  
        ...
        return (  
            f"{keyword_dir}/{crawler_type_var.get()}_{store_type}_{time_range}.json",  
            f"{keyword_words_dir}/{crawler_type_var.get()}_{store_type}_{time_range}"  
        )  
    ...
    async def save_data_to_json(self, save_item: Dict, store_type: str):  
        ...
        """  
        # 从save_item中获取关键词和时间范围  
        keyword = save_item.get("source_keyword", crawler_type_var.get())  
          
        # 获取时间范围，如果有START_DAY和END_DAY配置，则使用它们  
        time_range = None  
        if hasattr(config, 'START_DAY') and hasattr(config, 'END_DAY'):  
            time_range = f"{config.START_DAY.replace('-', '')}_{config.END_DAY.replace('-', '')}"  
